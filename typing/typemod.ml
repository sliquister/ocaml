...
let retype_applicative_functor_type env funct arg =
  ...
  ignore(Includemod.modtypes env
    ...
    { super with
      ...
      Btype.it_path = (fun referenced_path ->
        iter_path_apply referenced_path ~f:(fun funct arg ->
          if path_is_prefix ~strict:false path ~prefix:arg
          then
            let env = !env in
            try retype_applicative_functor_type env funct arg
...
let merge_constraint initial_env loc sg constr =
  ...
  try
    ...
    | (_, _, Twith_typesubst tdecl) ->
       let is_path_being_substituted =
         let rec loop rev_ids path =
           match rev_ids, path with
           | [], _ | _, Papply _ -> false
           | id :: _, Pident id' -> Ident.same id id'
           | id :: rev_ids, Pdot (p, n, _) ->
              Ident.name id = n && loop rev_ids p
         in
         loop (List.rev !real_ids) in
       let tdecl_body =
         match tdecl.typ_type.type_manifest with
         | None -> assert false
         | Some x -> x
       in
       let rewrite_type_constr_application path args =
         match is_path_being_substituted path with
         | false -> None
         | true ->
            let typ =
              Ctype.apply Env.empty
                tdecl.typ_type.type_params tdecl_body args
            in
            Some typ.desc
       in
       let rewrite_type_path path =
         if is_path_being_substituted path
         then
           Misc.fatal_error "Typemod.merge_constraint: not expecting to have \
                             to substitute a type in a position where a type \
                             expression isn't possible."
         else None
       in
       let sub =
         Subst.set_rewrite_paths Subst.identity
           { Subst.
    ...
    | (_, _, Twith_modsubst _) ->
       assert (List.length !real_ids = 1);
       let id = List.hd !real_ids in
       let path = match !real_path with None -> assert false | Some p -> p in
